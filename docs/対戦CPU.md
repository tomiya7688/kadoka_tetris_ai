...
標準CPU v3は `VisibleAttackPlacementPlanner` を使う。
...
v3では、探索深度内にある表示NEXTのTミノについても、安価な最終座標列挙ではなく到達可能なsemantic action探索を行う。
...
T-Spinは、現在ミノに加えて探索深度内の表示NEXTにあるTミノまで到達可能配置として評価できるようになった。
ただし、Tミノが探索深度よりさらに先にある場合にT-Spin穴を長期間維持する専用ヒューリスティックや、DT砲など特定テンプレートを認識する仕組みはまだ持たない。

まだ以下は専用探索していない。

- 探索深度外まで含めたT-Spin継続形・T-Spin穴維持の価値評価
- PC構築
