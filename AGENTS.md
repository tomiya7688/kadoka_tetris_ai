...
- 配布対象は、ユーザー側にPython・pip・IDE等の開発環境を要求せず、ZIP展開後にEXEを直接起動できる形を目標とする。
- 実装変更後は、可能な限り通常の自動テストに加えて配布ビルドを生成し、少なくとも起動確認・必須ファイル存在確認・設定/UserData初期化などの簡易スモークテストを実行する。
- Codexは作業完了前に、原則として「テスト → 配布ビルド → 配布物スモークテスト」の順で検証する。ビルド手段が未実装・破損している場合は、それを未検証として済ませず、配布基盤の未完了事項として明示して優先的に解消する。
